...
/**
 * @param elem    要生成 pdf 的 DOM 元素（容器）
 * @param pdfName PDF文件生成后的文件名字
 * */
const downloadPDF = (elem: HTMLElement, pdfName: string) => {
  const eleW = elem.offsetWidth;  // 获得该容器的宽
  const eleH = elem.offsetHeight;  // 获得该容器的高
  const eleOffsetTop = elem.offsetTop;  // 获得该容器到文档顶部的距离
  const eleOffsetLeft = elem.offsetLeft;  // 获得该容器到文档最左的距离
